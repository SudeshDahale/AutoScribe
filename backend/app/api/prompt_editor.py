...
async def _create_github_pr(
    ...
    """Create a GitHub PR with documentation content via GitHub REST API."""
    import httpx, base64
    ...
    branch = branch_name or f"autoscribe/{doc_type}-{ts}"
    title = pr_title or f"docs: Auto-update {doc_type.replace('_', ' ')} [{ts}]"

    async with httpx.AsyncClient(timeout=30) as client:
        # Get default branch SHA
        r = await client.get(f"{base_api}/git/ref/heads/{repo.default_branch}", headers=headers)
        ...
        if r.status_code not in (200, 201, 422):  # 422 = branch exists
            raise Exception(f"Could not create branch: {r.text}")

        # Check if file already exists (to get its sha for update)
        file_sha = None
        r = await client.get(f"{base_api}/contents/{target_file}?ref={branch}", headers=headers)
        if r.status_code == 200:
            file_sha = r.json().get("sha")

        # Commit file
        payload: dict = {
            "message": f"docs(autoscribe): update {target_file}",
        ...
        r = await client.post(
            ...
            json={
                ...
                "body": (
                    "## 📝 AutoScribe Documentation Update\n\n"
                    f"This PR was automatically generated by **AutoScribe**.\n\n"
                    f"**Document type:** `{doc_type}`\n"
                    f"**File updated:** `{target_file}`\n"
        ...
        if r.status_code not in (200, 201):
            raise Exception(f"Could not create PR: {r.text}")
